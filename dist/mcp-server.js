...
import { buildMemoryRecord, deleteMemory, estimateTokens, loadAllMemories, saveMemory, sanitizeStoreName, } from './memory-store.js';
// ---------------------------------------------------------------------------
...
export class MemoryManager {
    ...
    async storeMemory(params) {
        const record = buildMemoryRecord({
            ...
            memoryDetails: {
                ...
                source: 'agent',
                relatedUrls: params.metadata?.relatedUrls ?? [],
            },
            url: params.metadata?.url ?? null,
    ...
    async recallMemories(params) {
        ...
        let list = memories.filter((m) => !m.projectId || m.projectId === params.userId);
        if (params.category) {
    ...
    async searchMemories(userId, query, options) {
        ...
        let candidates = memories.filter((m) => !m.projectId || m.projectId === userId);
        if (options?.category) {
    ...
    async updateMemory(params) {
        const { memories } = await loadAllMemories(this.store);
        const existing = memories.find((m) => m.id === params.memoryId && (!m.projectId || m.projectId === params.userId));
        if (!existing) {
    ...
    async getMemoryStats(params) {
        const { memories } = await loadAllMemories(this.store);
        const userMemories = memories.filter((m) => !m.projectId || m.projectId === params.userId);
        const byType = {};
        const bySite = {};
        let totalTokens = 0;
        for (const m of userMemories) {
            ...
            if (m.site)
                bySite[m.site] = (bySite[m.site] ?? 0) + 1;
        ...
        return {
            ...
            oldestMemory: userMemories.length
                ? userMemories.reduce((a, b) => a.createdAt < b.createdAt ? a : b).createdAt
                : null,
     ...
     */
    async pruneMemories(userId, config) {
        const halfLifeDays = config?.halfLifeDays ?? 30;
        const pruneThreshold = config?.pruneThreshold ?? 0.1;
        const { memories } = await loadAllMemories(this.store);
        const userMemories = memories.filter((m) => !m.projectId || m.projectId === userId);
        const now = Date.now();
        const archived = [];
        for (const m of userMemories) {
 ...
 */
export async function startMcpServer(transport = 'stdio') {
    ...
    // ---- store_memory ----
    server.tool('store_memory', 'Store a new memory for a user. Use this when an agent learns something new about a site, project, or workflow and wants to persist it for future recall. Supports optional categorisation, tags, importance weighting, and arbitrary metadata.', {
    ...
    // ---- recall_memories ----
    server.tool('recall_memories', 'Retrieve stored memories for a user, optionally filtered by category. Returns the most recently updated memories first. Use this to surface context an agent has previously saved.', {
        userId: z.string().describe('Unique identifier for the user or project scope.'),
    ...
    // ---- search_memories ----
    server.tool('search_memories', 'Search memories using TF-IDF scoring. Use this when an agent needs to find relevant past knowledge by keyword or natural-language query. Results are ranked by relevance and recency.', {
        userId: z.string().describe('Unique identifier for the user or project scope.'),
    ...
    // ---- update_memory ----
    server.tool('update_memory', 'Update an existing memory by its ID. Use this to correct, refine, or augment a previously stored memory without creating a duplicate.', {
        userId: z.string().describe('Unique identifier for the user or project scope.'),
    ...
    // ---- delete_memory ----
    server.tool('delete_memory', 'Delete a specific memory by its ID. Use this when a memory is no longer relevant or was stored incorrectly and needs to be removed entirely.', {
        userId: z.string().describe('Unique identifier for the user or project scope.'),
    ...
    // ---- get_memory_stats ----
    server.tool('get_memory_stats', 'Get statistics about stored memories for a user, including counts by type and site, token estimates, and date ranges. Useful for monitoring memory health and deciding when to prune.', {
        userId: z.string().describe('Unique identifier for the user or project scope.'),
    }, async (args) => {
        try {
    ...
    // ---- prune_memories ----
    server.tool('prune_memories', 'Trigger temporal decay and prune low-scoring memories. Memories are scored using exponential decay based on age and a configurable half-life. Those below the prune threshold are archived and removed. Use this periodically to keep the memory store relevant.', {
        userId: z.string().describe('Unique identifier for the user or project scope.'),
