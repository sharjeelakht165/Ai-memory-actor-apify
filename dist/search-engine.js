...
export class SearchEngine {
    ...
    search(query, memories, options = {}) {
        const { limit = 20, memoryType, includeArchived = false, minScore = 0, projectId, } = options;
        // Filter memories
        ...
        const docTerms = candidates.map((m) => this.tokenize(`${m.title} ${m.content} ${m.tags.join(' ')} ${m.memoryType}`));
        ...
        const results = candidates.map((memory, idx) => {
            ...
            const tagBoost = memory.tags.some((tag) => queryTerms.some((qt) => tag.toLowerCase().includes(qt))) ? 0.15 : 0;
            // URL/site relevance bonus
            const urlBoost = memory.url && queryTerms.some((qt) => memory.url && memory.url.toLowerCase().includes(qt)) ? 0.1 : 0;
